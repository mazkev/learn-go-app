...
export default function MobileLearningHub({
  ...
  const languages = [
    ...
    { id: "javascript", name: "JS/TS", icon: "🟨", color: "#F7DF1E" },
  ...
  return (
    <div className="md:hidden flex flex-col min-h-screen bg-slate-50 dark:bg-[#070d19] pb-24 animate-fadeIn">
      {/* 1. Mobile Hero Header: Daily Stats & Quick Streak */}
      <div className="p-4 bg-gradient-to-b from-white to-slate-50 dark:from-[#0d1527] dark:to-[#070d19] border-b border-slate-200 dark:border-white/10 space-y-3">
        {/* Top Mini Stats Bar */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1 rounded-2xl bg-orange-500/10 text-orange-600 dark:text-orange-400 text-xs font-mono font-black border border-orange-500/30 shadow-2xs cursor-pointer active:scale-95 transition-transform"
            >
            ...
            {/* Level & XP */}
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-2xl theme-card-subtle text-xs font-mono font-bold border border-slate-200 dark:border-white/10">
        ...
        <div>
          <span className="text-[10px] font-mono uppercase tracking-wider theme-muted font-bold block mb-1.5">
            ...
            {languages.map((lang) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-bold font-mono shrink-0 transition-all cursor-pointer border ${
                    isSelected
                      ? "bg-slate-900 text-white dark:bg-white dark:text-slate-950 border-transparent shadow-md scale-105"
                      : "theme-card-subtle theme-heading border-slate-200 dark:border-white/10 hover:bg-black/5"
                  }`}
                >
                  <span className="text-sm">{lang.icon}</span>
                  <span>{lang.name}</span>
                  {isSelected && <span className="w-1.5 h-1.5 rounded-full bg-[#04AA6D]" />}
                </button>
              );
            })}
          </div>
        ...
        <div className="p-4">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#04AA6D] via-teal-600 to-emerald-800 text-white p-5 shadow-xl shadow-[#04AA6D]/20">
            ...
            <div className="relative z-10 space-y-3">
              <div className="flex items-center justify-between">
                <span className="px-2.5 py-0.5 rounded-full bg-white/20 backdrop-blur-md text-[10px] font-mono font-extrabold uppercase tracking-wider">
      ...
      <div className="px-4 space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-mono font-black uppercase tracking-wider theme-heading flex items-center gap-1.5">
        ...
        <div className="space-y-3">
          {modules.map((mod, modIdx) => {
            ...
            return (
              <div
                key={mod.id}
                className="theme-card rounded-3xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-sm transition-all"
              >
                {/* Module Header Card */}
                <div
                  onClick={() => toggleModule(mod.id)}
                  className="p-4 flex items-center justify-between gap-3 cursor-pointer select-none active:bg-black/5 dark:active:bg-white/5"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-sm shrink-0 shadow-sm ${
                        isModFullyCompleted
                          ...
                          : "theme-card-subtle theme-heading"
                      }`}
                    >
                    ...
                    <div className="min-w-0">
                      <h5 className="text-sm font-extrabold theme-heading truncate">
                        {mod.title}
                      </h5>
                      <div className="flex items-center gap-2 text-[11px] font-mono theme-muted mt-0.5">
                  ...
                  <ChevronDown
                    size={18}
                    className={`theme-muted transition-transform duration-200 ${
                ...
                {isExpanded && (
                  <div className="px-3 pb-3 pt-1 space-y-1.5 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/20">
                        ...
                        <div
                          ...
                          className={`p-3 rounded-2xl flex items-center justify-between gap-2 cursor-pointer transition-all ${
                            isCurrent
                              ...
                              : "theme-card hover:bg-white/80 dark:hover:bg-white/5 border border-slate-200/60 dark:border-white/5"
                          }`}
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              <span className="text-xs font-bold theme-heading block truncate">
                                {lesson.title}
                              </span>
                              <span className="text-[10px] theme-muted truncate block">
                                {lesson.summary}
                              </span>
                            </div>
                          </div>
